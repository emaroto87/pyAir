...
def XYZ(i: int, j: int, wij: float, L: float, B: float, points_wave=9):
    ...
    x = np.linspace(0, L, points_wave)
    y = np.linspace(0, B, points_wave)
    x, y = np.meshgrid(x, y)
    z = wij * np.sin(alpha(i)*x) * np.sin(beta(j)*y)
    return x, y, z

def XYZ_all(wij_vector: np.array , nx: int, ny: int, L: float, B: float, points_wave=9):
    ...
    for i in range(1, nx+1):
        for j in range(1, ny+1):
            ...
            z += coeff * np.sin(alpha(i)*x) * np.sin(beta(j)*y)
    return x,y,z
...
def draw(x: np.array, y: np. array, z: np.array, scale_factor=1.0):
    ...
    ax.set_box_aspect((L,B,B*0.3))
    surf = ax.plot_surface(x, y, z, vmin=z.min() * 2, cmap=cm.rainbow)
    ax.set_xlabel('Length (L)')
    ax.set_ylabel('Width (B)')
    ax.set_zlabel('Shape (W)')
...
def ex2_check(mode):
    ...
    eigenvalues, eigenvectors = solve_eig_tikhonov(1e-6)
...
def ex3_check(mode):
    ...
    wij_array_mode = eigenvectors[:,mode]
    main_w_mode, mode_i, mode_j = eigmode(nx, ny, wij_array_mode)
    # x, y, z = XYZ(mode_i, mode_j, main_w_mode, L, B)
    print(mode_i,mode_j)
    x, y, z = XYZ_all(wij_array_mode ,nx,ny, L, B,20)
    draw(x, y, z)
