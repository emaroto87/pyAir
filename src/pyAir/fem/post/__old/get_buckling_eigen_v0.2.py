...
from common.general import merge_dfs
...
def get_buckling_eigen(op2_files_path: 'list[str]')->'dict{dataframes}':
    '''
    Generates a dictionary of tables from a list of NASTRAN OP2 files containing
    ...
    for op2_file in op2_files_path:
        ...
        for sid,eigenvector in op2.eigenvectors.items():
            ...
            minpos_eigen_value = np.where(eigenvalues>0,eigenvalues,np.inf).min()
        ...
        columns = ['eigrval_{0}'.format(eigrval_index+1) for eigrval_index in range(n_eigrvalues)]
        columns += ['min_pos']
...
if __name__ == '__main__':
    op2_paths = ask_op2_filepath(multiple=True)
    eigrs_tables = get_buckling_eigen(op2_paths)
    excel_path = asksaveasexcelfile()[0]
    # export_tables_to_excel(eigrs_tables,excel_path)
    to_excel(eigrs_tables,excel_path,True,True)